...
namespace EngineeringDiscovery.Wpf.Tests
{
    public class EngineeringWorkspaceTests
    {
        ...
        public void EngineeringWorkspace_XamlDefines_EngineOsHierarchyAndConversation()
        {
            ...
            Assert.Contains("Conversation will execute only the confirmed Build or Test capability", content);
            ...
            Assert.Contains("HorizontalScrollBarVisibility=\"Auto\"", content);
            Assert.Contains("CanContentScroll=\"False\"", content);
            Assert.Contains("<views:DevelopmentSurface DataContext=\"{Binding DevelopmentSurface}\"", content);
            Assert.DoesNotContain("<views:DevelopmentSurface Grid.Row=\"2\"", content);
